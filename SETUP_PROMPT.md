...
3. **GitHub sign-in.** Run `gh auth status`. If I'm not signed in, ask me to run `gh auth login` myself (choose HTTPS and let it set up git credentials), then continue. Unless I have write access to the shared repo, contributions go through a fork under my own account, created automatically the first time.
...
7. **Explain how to use it.** In a few lines, tell me:
   ...
   - Paste a YouTube tutorial link to ingest it. Claude writes the notes, keeps them on this machine, and opens a pull request. The repo owner reviews and merges it, and then everyone else gets it on their next sync.
